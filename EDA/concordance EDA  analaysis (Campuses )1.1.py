...
#%%
#bar plot between location name and percentage of scores in each location for 1,2,3,4,5

#Use the groupby function to group the data by 'location' and count the number of occurrences of each 'score'
grouped = data.groupby(['LocationName', 'Score']).size().reset_index(name='count')

#grouped1=data.gro'LocationName'upby(['LocationName', 'Score']).size().reset_index(name='count')
unique_loc=list(grouped['LocationName'].unique())

score_percent=[]
location=[]
for loc in unique_loc :
    
    count=grouped[grouped['LocationName']==loc]['count'].sum()
    score_percent.append(count)
    location.append(loc)

grouped1=pd.DataFrame({'LocationName':location,
                       'total_count':score_percent})    
    
grouped=pd.merge(grouped,grouped1,how='left',on='LocationName')

grouped['percentage']=grouped['count']/grouped['total_count'] * 100
    
#grouped['percentage']=grouped['count']/grouped['count'].sum() * 100
# Create the bar plot
sns.barplot(x='LocationName', y='percentage', hue='Score', data=grouped)

# Add labels and titles to the plot
plt.xlabel('Location')
plt.ylabel('percentage of Score')
plt.title('Score percentage by Location')
...
# Perform ANOVA test
f, p = stats.f_oneway(grouped_df['Score'].loc[grouped_df['LocationName'] == 'Miami Campus'], 
                      grouped_df['Score'].loc[grouped_df['LocationName'] == 'Gatwick Campus'], 
                      grouped_df['Score'].loc[grouped_df['LocationName'] == 'Singapore Campus'])

# Create a significant difference plot
sns.boxplot(x='LocationName', y='Score', data=grouped_df)

if p < 0.05:
    print("There is a significant difference in the scores between the locations.")
else:
    print("There is no significant difference in the scores between the locations.")
...
unique_ins=list(grouped['InstructorBEMSID'].unique())

score_percent=[]
instructor=[]
for ins in unique_ins :
    
    count=grouped[grouped['InstructorBEMSID']==ins]['count'].sum()
    score_percent.append(count)
    instructor.append(ins)

grouped1=pd.DataFrame({'InstructorBEMSID':instructor,
                       'total_count':score_percent})    
    
grouped=pd.merge(grouped,grouped1,how='left',on='InstructorBEMSID')

grouped['percentage']=grouped['count']/grouped['total_count'] * 100

# Create the bar plot with the percentage values
sns.barplot(x='InstructorBEMSID', y='percentage', hue='Score', data=grouped.head(30))

# Add labels and titles to the plot
plt.xlabel('Instructor with in Miami Campus')
plt.ylabel('Percentage of Score')
plt.title('Score Percentage by Instructor')
...
#Use the groupby function to group the data by 'location' and count the number of occurrences of each 'score'
grouped = data.groupby(['LocationName','ObjectiveSkill', 'Score']).mean().reset_index()

sns.barplot(x='ObjectiveSkill', y='Score', data=grouped, hue='LocationName')

plt.title('Location vs Average Objective skill Score')
plt.xlabel('ObjectiveSkill')
plt.ylabel('Average Objective skill Score')
